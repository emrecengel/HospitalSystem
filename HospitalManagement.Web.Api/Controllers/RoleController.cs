...
using HospitalManagement.Services.Modules.RolesModule.Queries;
using MediatR;
    ...
    IMediator mediator) : ControllerBase
{
    ...
    public Task<List<Role>> Query([FromQuery] QueryRole query)
    {
    ...
    [HttpGet("Find")]
    [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Role))]
    public async Task<IActionResult> Find([FromQuery] FindRole query)
    {
    ...
    {
        var record = await mediator.Send(new FindRole().ById(id));
    ...
    {
        await mediator.Send(new DeleteRole().UseId(id));
